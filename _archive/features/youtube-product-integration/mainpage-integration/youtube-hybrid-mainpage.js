/*
 * =============================================
 * YouTube 하이브리드 레이아웃 - 메인페이지 통합 JS
 * =============================================
 *
 * 기능:
...
(function() {
  ...
  function init() {
    log('[YouTube-Hybrid] 초기화 시작');

    // 컨테이너 확인
    if (!document.getElementById('youtube-main-area')) {
      log('[YouTube-Hybrid] 컨테이너 없음, 초기화 중단');
      return;
    }
    ...
    loadMappingData()
      ...
      .catch(function(error) {
        console.error('[YouTube-Hybrid] 초기화 실패:', error);
        showErrorMessage();
      });
  }
  ...
  function loadMappingData() {
    return new Promise(function(resolve, reject) {
      log('[Mapping] 데이터 로드 시작: ' + CONFIG.mappingDataUrl);
      ...
      $.ajax({
        ...
        error: function(xhr, status, error) {
          log('[Mapping] 데이터 로드 실패 (' + error + '), 제품 연동 없이 진행');
          resolve();
        }
      });
    });
  ...
  function loadYouTubeVideos() {
    var CACHE_KEY = 'yt_hybrid_cache';
    var CACHE_TIME_KEY = 'yt_hybrid_time';
    ...
    try {
      ...
      if (cachedData && cacheTime && (now - parseInt(cacheTime)) < CONFIG.cacheDuration) {
        log('[YouTube] 캐시 사용 (만료까지 ' +
            Math.round((CONFIG.cacheDuration - (now - parseInt(cacheTime))) / 3600000) + '시간)');
  ...
  function renderLayout() {
    ...
    renderMoreVideos();
    log('[YouTube-Hybrid] 레이아웃 렌더링 완료');
  }
  ...
  function renderMainArea(video) {
    ...
    html += '  <h4 class="main-video-title">' + escapeHtml(video.title) + '</h4>';
    ...
    html += '  <div class="product-sidebar-header">영상 속 재료 구매하기</div>';
  ...
  function createThumbnailCard(video, index) {
    ...
    html += '    <p class="thumb-title">' + escapeHtml(truncateTitle(video.title, 35)) + '</p>';
    html += '  </div>';
    html += '</div>';
    ...
    card.addEventListener('click', function() {
      ...
      // 스크롤 이동
      var section = document.querySelector('.youtube-hybrid-section');
      if (section) {
  ...
  function showErrorMessage() {
    ...
    var moreSection = document.querySelector('.more-videos-section');
    if (moreSection) {
  ...
  window.enableYouTubeHybridDebug = function() {
    CONFIG.debug = true;
    console.log('[YouTube-Hybrid] 디버그 모드 활성화');
    console.log('[YouTube-Hybrid] 현재 설정:', CONFIG);
    console.log('[YouTube-Hybrid] 로드된 영상:', allVideos.length + '개');
    console.log('[YouTube-Hybrid] 매핑 데이터:', mappingData ? Object.keys(mappingData.products).length + '개 제품' : '없음');
  };

  // 캐시 삭제
  window.clearYouTubeHybridCache = function() {
    localStorage.removeItem('yt_hybrid_cache');
    localStorage.removeItem('yt_hybrid_time');
    console.log('[YouTube-Hybrid] 캐시 삭제 완료. 새로고침합니다.');
    location.reload();
  };

  // 수동 새로고침
  window.refreshYouTubeHybrid = function() {
    localStorage.removeItem('yt_hybrid_cache');
    localStorage.removeItem('yt_hybrid_time');
    currentMainIndex = 0;
    allVideos = [];
